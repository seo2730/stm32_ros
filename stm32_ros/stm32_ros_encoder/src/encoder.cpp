...
#include "std_msgs/UInt32.h" // MsgTutorial 메시지 파일 헤더 (빌드 후 자동 생성됨)
...
void msgCallback(const std_msgs::UInt32::ConstPtr& msg)
{
	ROS_INFO("encoder = %d", msg->data);// data 메시지를 표시한다
}

int main(int argc, char **argv)// 노드 메인 함수
{
	ros::init(argc, argv, "encoder_subscriber");// 노드명 초기화
	...
	ros::Subscriber encoder_sub = nh.subscribe("encoder", 100, msgCallback);
